...
app.get('/realizar-venta', verificarSesion, (req, res) => {
    ...
    const productosQuery = `SELECT * FROM InventarioProductos WHERE NombreProducto LIKE '%${searchTerm}%'`;
        ...
        connection.query(productosQuery, (errorProductos, resultsProductos) => {
            if (errorProductos) {
                ...
                if (resultsProductos.length === 0) {
                    res.json({ error: 'No se encontró información para el producto.' });
                } else {
                    // Enviar los detalles del producto en formato JSON
                    res.json({
                        stock: resultsProductos[0].CantidadStock,
                        precio: parseFloat(resultsProductos[0].PrecioProducto)
                    });
                }
            }
        });
...
app.get('/buscar-productos', verificarSesion, (req, res) => {
    const searchTerm = req.query.searchTerm || ''; // Obtener el término de búsqueda del usuario
    const productosQuery = `SELECT * FROM InventarioProductos WHERE NombreProducto LIKE '%${searchTerm}%'`;

    console.log('Término de búsqueda:', searchTerm); // Agregar mensaje de registro
    ...
    connection.query(productosQuery, (errorProductos, resultsProductos) => {
        if (errorProductos) {
            ...
            console.log('Resultados de búsqueda de productos:', resultsProductos); // Agregar mensaje de registro
            res.json({ productos: resultsProductos });
        }
    });
});
...
app.get('/obtener-productos', verificarSesion, (req, res) => {
    ...
    connection.query(productosQuery, (errorProductos, resultsProductos) => {
        if (errorProductos) {
            console.error('Error al obtener los productos:', errorProductos);
            res.json({ success: false, error: 'Error al obtener los productos.' });
        } else {
            ...
            res.json({ success: true, productos: listaProductos });
        }
    });
});
...
app.post('/guardar-venta', (req, res) => {
    ...
    try {
        ...
        for (const productoID in productosAgregadosObj) {
            console.log(`cada prodcutoid: ${productoID}`);
            if (productosAgregadosObj.hasOwnProperty(productoID)) {
                ...
                detallesVenta.push({
                    ...
                    PrecioUnitario: producto.precio, // Agregar el precio aquí
        ...
        const queryVenta = 'INSERT INTO Ventas (Cliente, IDEmpleado, FechaVenta, TotalVenta, TotalExtra, TotalFinal, MetodoPago) VALUES (?, ?, ?, ?, ?, ?, ?)';
        const valuesVenta = [cliente, IDEmpleado, fechaVenta, totalVentaCalculado, totalExtra, totalFinalNumerico, metodoPago];
